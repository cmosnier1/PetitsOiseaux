...
function applyFirebaseData(rawData) {
  isSyncingFromFirebase = true;

  try {
    // Restaurer les clés originales (ex: "Santé%2FPharma" → "Santé/Pharma")
    const data = deepRestoreFromFirebase(rawData);
...
/**
 * Encode les caractères interdits dans les clés Firebase
 * Firebase interdit : . # $ / [ ]
 */
 ...
 * - Encode les caractères interdits dans les clés
 * - Convertit les tableaux creux en tableaux denses
 */
function deepCleanForFirebase(data) {
  ...
  for (const key of Object.keys(data)) {
    const val = data[key];
    if (val !== undefined) {
      cleaned[sanitizeKey(key)] = deepCleanForFirebase(val);
    }
  }
  return cleaned;
}
...
/**
 * Restaure les clés originales après lecture depuis Firebase
 */
function deepRestoreFromFirebase(data) {
  if (data === null || data === undefined || typeof data !== 'object') return data;

  if (Array.isArray(data)) {
    return data.map(item => deepRestoreFromFirebase(item));
  }

  const restored = {};
  for (const key of Object.keys(data)) {
    restored[desanitizeKey(key)] = deepRestoreFromFirebase(data[key]);
  }
  return restored;
}
